...
def test_show_null():
    ui = AutoObjectForm.from_pydantic_model(
        CoreIpywidgets
    )  # NOTE: CoreIpywidgets has a Nullable values
...
def test_show_title_description():
    ...
    class Compound(BaseModel):
        ...
        nest_str_1: Annotated[
            ...
            Field(
                ...
                description="Overridden description nest_str_1, title not coming through",
            ),
        ]
    ...
    for k in ui.schema["properties"].keys():
        property = (
            lambda ui, k: (
                ui.schema["properties"][k]["allOf"][0]
                if "allOf" in ui.schema["properties"][k]
                else ui.schema["properties"][k]
            )
        )(ui, k)
