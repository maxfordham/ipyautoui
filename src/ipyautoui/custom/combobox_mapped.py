...
class ComboboxMapped(w.Box):
    ...
    def value(self, value):
        if value in [o[0] for o in self.options]:
            ...
            self.widget.value = self.options[[o[1] for o in self.options].index(value)][0]
        else:
    ...
    def validate_options(options):
        ...
        elif isinstance(options, list) and len(options)>1 and isinstance(options[0], tuple):
            return options
        elif isinstance(options, list) and len(options)>1 and not isinstance(options[0], tuple):
