...
from ipyautoui.constants import BUTTON_WIDTH_MIN, TRUE_BUTTON_KWARGS, FALSE_BUTTON_KWARGS, DASH_BUTTON_KWARGS
...
class FindFiles(widgets.VBox, HasTraits):
    ...
    def __init__(self, 
        ...
        if match_files is None: 
            ...
            self.cls_listfiles = functools.partial(MatchStrings, match_strings= match_files)
        self.title = widgets.HTML(title)
        self._init_form()
        self.recursive = recursive
        self.pydantic_obj = FilesInDir(fdir=fdir, patterns=patterns, recursive=recursive)
    ...
    def fn_add_pattern(self, value=None):
        if self.editable_patterns:
            ...
            return widgets.Text(placeholder='add regex glob pattern', value=value, disabled=True)
    ...
    def _fdir(self, on_change):
        self.pydantic_obj = FilesInDir(fdir=self.fdir_ui.value, patterns=self.pydantic_obj.patterns, recursive=self.recursive)
        
    def _refresh(self, on_click):
        self.pydantic_obj = FilesInDir(fdir=self.fdir, patterns=self.patterns, recursive=self.recursive)
    ...
    def _init_form(self):
        ...
        self.refresh = widgets.Button(icon='refresh', layout={'width':BUTTON_WIDTH_MIN}, tooltip='update file list (search folder again)')
        self.box_title.children = [self.refresh, self.title]
        
        # fdir
        self.box_fdir = widgets.HBox()
        self.box_folder = widgets.HBox([widgets.HTML('<b>folder:</b> ', layout={'width':width}), self.box_fdir])

        # patterns
        self.patterns_ui = Array(show_hash=None)
        self.box_patterns = widgets.HBox([widgets.HTML('<b>search patterns:</b> ', layout={'width':width}), self.patterns_ui])

        # fpths
        self.fpths_ui = self.cls_listfiles(value = [])
        self.box_fpths = widgets.HBox([widgets.HTML('<b>filepaths:</b> ', layout={'width':width}), self.fpths_ui])

        # box
        super().__init__()
        self.children = [self.box_title, self.box_folder, self.box_patterns, self.box_fpths]
...
if __name__ == "__main__":
    ...
    ff = FindFiles(fdir=fdir, patterns=patterns, recursive=True, title='<b>find files in folder</b>', match_files=None) #, editable_fdir=True
    display(ff)
